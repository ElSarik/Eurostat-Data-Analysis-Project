...
def cleaner(dataset_path, gdp_path=None, remove_negative=True, cofog=False, save=False):
    ...
    information = pd.read_excel(
        dataset_path, sheet_name=None, skiprows=3, nrows=4, usecols="A, B, C"
    )

    # Loading the data
    data = pd.read_excel(dataset_path, sheet_name=None, skiprows=9)
    ...
    for key in keys:
        ...
        df.replace(
            "European Union - 27 countries (from 2020)", "European Union", inplace=True
        )
    ...
    if remove_negative:
        ...
        for idx, row in negatives_info.iterrows():
            ...
            division = [key for key, value in cofog_data.items() if category in value][
                0
            ]

            category_filter = dataset.loc[
            ...
            dataset.loc[
                (
                    (dataset["Country"] == country)
                    & (dataset["Year"] == year)
                    & (dataset["Category"] == division.lower())
                ),
                ["Million euro"],
            ] = category_total

            division_filter = dataset.loc[
                (dataset["Country"] == country)
                & (dataset["Year"] == year)
    ...
    if gdp_path is not None:
        # Loading the data
        data = pd.read_excel(gdp_path, sheet_name=None, skiprows=8)
        ...
        for key in keys:
            ...
            gdp.replace(
                "Euro area - 19 countries  (from 2015)", "Eurozone", inplace=True
            )
            gdp.replace(
    ...
    if cofog:
        ...
        if gdp_path is not None:
            ...
            dataset = dataset[
                ["Country", "Year", "Million euro", "% GDP", "Division", "Category"]
            ]
